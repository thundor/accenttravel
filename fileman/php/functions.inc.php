...
function t($key){
  ...
  if(empty($MLANG)){
    ...
    $file = $langPath.$file;
    $MLANG = json_decode(file_get_contents($file), true);
  }
  // echo '<pre>';
  // print_r(get_class_methods($LANG));
  // print_r($LANG);
  // die;
  if(!$MLANG[$key])
    $MLANG[$key] = $key;
...
function getFilesPath(){
  ...
  if(!$ret){
	  ...
	  $d = trim(trim($d, '/'));
	  // $d = 'resources';
	  if('' === $d || (preg_match('/\.\//', $d) || !is_dir($root_dir . '/' . $d))){
		  $d = '/resources';
	  } else {
		  $d = '/' . $d;
	  }
    ...
    $ret = RoxyFile::FixPath($root_dir . $d);
...
class RoxyFile{
  ...
  static function IsImage($fileName){
    ...
    if($ext == 'webp' || $ext == 'jpg' || $ext == 'jpeg' || $ext == 'jpe' || $ext == 'png' || $ext == 'gif' || $ext == 'ico')
      $ret = true;
    return $ret;
  }
