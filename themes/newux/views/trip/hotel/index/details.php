...
?>
<v-container class="px-0 pt-3" fluid v-if="(mydata.mystep = (mydata.step < 3 ? 2 : mydata.step) || 2)">
	...
	<v-window-item :value="0" class="w-100 fill-height">
		...
		<v-window id="search-wrapper-windows" v-model="mydata.mystep" :touch="false">
			<v-window-item :value="2">
				<component :is="loadViewAsync('partials/presearch-wrapper/functionalities/trip-hoteluri/hotel')" :data="mydata" :defaults="<?php echo htmlspecialchars(json_encode(['trip-hoteluri' => $hotel_search_data]), ENT_QUOTES); ?>" :hotel="<?php echo htmlspecialchars(json_encode($this->view_data['hotel_details']), ENT_QUOTES); ?>"></component>
			</v-window-item>
