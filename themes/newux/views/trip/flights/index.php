...
if($special_layout){
  ...
  if(isset($class) && is_numeric($class) && isset($this->_ci->Flights_model->cabin_type_map[(int)$class])){
    ...
    // $search_data['cabine_type'] = (int)$class;
  }
	$direct = $this->_ci->input->get('direct');
  if(isset($direct)){
    $search_data['direct_only'] = filter_var($direct, FILTER_VALIDATE_BOOLEAN);
  }
	$go = $this->_ci->input->get('go');
  if(isset($go)){
    $go_only = filter_var($go, FILTER_VALIDATE_BOOLEAN);
  }
	$search_data['type'] = $go_only ? 0 : 1;
	$flex = $this->_ci->input->get('flex');
  if(isset($flex)){
    $search_data['flexible_dates'] = filter_var($flex, FILTER_VALIDATE_BOOLEAN);
  }
  ...
  foreach(array('origin', 'destination') as $param){
    ...
    if(isset($g_param)){
      ...
      if($response){
        ...
        // $search_data[$param . '_' . 'city_id'] = $first_location->CityId;
...
<v-window class="" :touch="false">
	<v-window-item :value="0" class="w-100 fill-height">
		...
		>
			<component :is="loadViewAsync('partials/search-wrapper')" activate_menu="trip-avion" :defaults="<?php echo htmlspecialchars(json_encode(['trip-avion' => $search_data]), ENT_QUOTES); ?>">
