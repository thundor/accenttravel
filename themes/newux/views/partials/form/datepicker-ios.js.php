...
class IosSelector {
  constructor(options) {
    let defaults = {
      ...
      count: 10, // 圆环规格，圆环上选项个数，必须设置 4 的倍数
      sensitivity: 0.4, // 灵敏度
    ...
    this.selected = this.value ? this.source.find((v) => v.value == this.value) : this.source[0];
    ...
    this.elems = {
      el: this.options.element || document.querySelector(this.options.el),
      circleList: null,
  ...
  select(value, instant) {
	...
	if(this.source[0]){
		return this.select(this.source[0].value, instant)
	}
...
export default {
  ...
  props: {
	...
	template : `
<div class="v-theme--dark date-selector" role="button">
  ...
  mounted: function() {
      function getYears(minmax) {
		let currentYear = minmax[1] || new Date().getFullYear();
		let years = [];
		
		var diff = minmax[1] ? minmax[1] - (minmax[0] || (minmax[1] - 20)) : 20;

		for (let i = currentYear - diff; i <= currentYear; i++) {
			years.push({
				value: i,
				text: i
			});
	...
	let yearSource = getYears(this.yearRange);
	let monthSource = getMonths();
	let daySource = getDays(currentYear, currentMonth);
	// console.warn(this.yearRange, this.yearRange.map((v) => {return {value: v, text: v}}));
	
	var date = this.modelValue ? [this.modelValue.getFullYear(), this.modelValue.getMonth() + 1, this.modelValue.getDate()] : undefined;
	//console.warn(date);
	yearSelector = new IosSelector({
		element: this.$refs.year1,
		type: 'infinite',
		source: yearSource,
		count: 15,
		value: date ? date[0] : null,
		onChange: (selected) => {
			currentYear = selected.value;
			daySource = getDays(currentYear, currentMonth);
			if(daySource.length != daySelector.source.length){
				daySelector.updateSource(daySource);
			}
			this.setValue([yearSelector.value, monthSelector.value, daySelector.value]);
		}
	});

	monthSelector = new IosSelector({
		element: this.$refs.month1,
		type: 'infinite',
		source: monthSource,
		count: 15,
		value: date ? date[1] : null,
		onChange: (selected) => {
	...
	daySelector = new IosSelector({
		element: this.$refs.day1,
		type: 'infinite',
		source: daySource,
		count: 15,
		value: date ? date[2] : null,
		onChange: (selected) => {
