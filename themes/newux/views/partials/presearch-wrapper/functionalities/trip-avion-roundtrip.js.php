...
export default {
	...
	data: () => {
		return {
			...
			menu: {
				title: 'Avion Roundtrip',
				icon: 'mdi-airplane-search',
				backgroundImage: 'https://accenttravel.ro/resources/images/Tema/turcia.webp',
			},
			functionalities: [
				'type',
				// 'cabin',
	...
	methods: {
		clickSelected: function(who, what){
			...
			this.$nextTick(() => {
				console.warn("CLICKSELECTED", '<?php echo basename(__FILE__, '.js.php'); ?>/', this.$options.name);
	...
	computed: {
		filtered_functionalities() {
			return (this.functionalities || []).filter(f => {
				if((this.data['<?php echo basename(__FILE__, '.js.php'); ?>/type'] || {}).Id == '0'){
					if(f == 'check-out') return false;
				}
				return true;
	...
	created() {
		if(this.content_type == 'default'){
			...
			if(!isNaN(step) && step > 0){
				console.warn('tripavion setting step', step);
				...
				<teleport :to="!search_wrapper_step ? '#<?php echo basename(__FILE__, '.js.php'); ?>-menu' : '#search-wrapper-step-1'">
					...
					<template v-slot:before="{ active_menu }" v-if="search_wrapper_step ==1 && data['<?php echo basename(__FILE__, '.js.php'); ?>/destination-city']">
						<v-container class="section-header" v-for="result in [data['<?php echo basename(__FILE__, '.js.php'); ?>/destination-city']]">
							<div class="text-h4">Oferte Zboruri <span v-text="[result.Name, result.Destination, result.Country].filter(v => !!v).join(' - ')"></span></div>
							<FormLegend :title="(data['<?php echo basename(__FILE__, '.js.php'); ?>/destination-city'].City || data['<?php echo basename(__FILE__, '.js.php'); ?>/destination-city'].Name) + ' este o alegere grozava! Iata si rezultatele cautarii tale. Vezi care pachet ti se pare potrivit si de indata ce ai ales, nu uita sa faci rezervarea!'"></FormLegend>
						</v-container>
					</template>
					</component>
					<template v-if="display_content && content_type == 'default'">
						<teleport to="#search-wrapper-content">
						<component :is="-1 == data?.['<?php echo basename(__FILE__, '.js.php'); ?>/type']?.Id ? loadViewAsync('partials/presearch-wrapper/functionalities/<?php echo basename(__FILE__, '.js.php'); ?>/search-roundtrip') : loadViewAsync('partials/presearch-wrapper/functionalities/<?php echo basename(__FILE__, '.js.php'); ?>/search')" :data="data" v-on:set-value="(v) => $.emit('set-value', v)" v-on:click-selected="clickSelected" :key_path="(key_path ? key_path + '.' : '') + key"></component>
						</teleport>
					</template>
				</teleport>
			</template>
			<?php /* <template v-slot:before="{ active_menu }" v-if="data['<?php echo basename(__FILE__, '.js.php'); ?>/destination-city']">
				<FormLegend :title="(data['<?php echo basename(__FILE__, '.js.php'); ?>/destination-city'].City || data['<?php echo basename(__FILE__, '.js.php'); ?>/destination-city'].Name) + ' este o alegere grozava! Iata si rezultatele cautarii tale. Vezi care pachet ti se pare potrivit si de indata ce ai ales, nu uita sa faci rezervarea!'" class="bg-white rounded-lg"></FormLegend>
			</template> */ ?>
		</component>
