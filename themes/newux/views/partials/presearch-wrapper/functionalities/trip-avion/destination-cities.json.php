...
if(!empty($_GET['force_new']) || !is_file($assets_dir . '/trip-cities-air.json.gz')){
	$fp = fopen($assets_dir . '/trip-cities-air-partial.json', 'w');
	fwrite($fp, '[');
	$has_results = true;
	$limit = 10000;
	$page = 0;
	...
	fwrite($fp, ']');
	fclose($fp);
	rename($assets_dir . '/trip-cities-air-partial.json', $assets_dir . '/trip-cities-air.json');
	gzCompressFile($assets_dir . '/trip-cities-air.json');
}

header("Content-Encoding: gzip");
$fp = fopen($assets_dir . '/trip-cities-air.json.gz', 'rb');
fpassthru($fp);
fclose($fp);
return;