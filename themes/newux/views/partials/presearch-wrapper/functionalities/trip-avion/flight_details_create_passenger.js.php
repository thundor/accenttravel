...
import IOSDatepicker from '../../../form/datepicker-ios.js?newux=<?php echo !empty($_GET['newux']) ? (string)$_GET['newux'] : 1 ; ?>';
export default {
  ...
  components:{
    ...
    date: undefined,
    today: new Date(),
    exp_date: new Date(new Date().setYear(new Date().getFullYear() + 60)),
    dob: undefined,
    nationality: '',
    dialog: false,
    errors: [],
    ...
    validations:Object.freeze([
		function(){ return !this.dob ? 'required' : null },
		...
		function(){ 
			...
			if(this.passport_required && this.doctype == '1'){
				return "Pentru acest zbor este obligatoriu Pasaportul";
			}
			if(this.doctype == '1'){
				return this.ci_s 
					&& this.ci_e 
					&& this.ci_s.toISOString().split('T')[0] < this.ci_e.toISOString().split('T')[0] 
					&& this.ci_s.toISOString().split('T')[0] <= n.toISOString().split('T')[0] 
					&& this.ci_e.toISOString().split('T')[0] <= d_max.toISOString().split('T')[0] 
					&& this.ci_e.toISOString().split('T')[0] >= de.toISOString().split('T')[0] 
			...
			if(this.doctype == '0'){
				...
				return this.pass_s 
					...
					&& this.pass_s.toISOString().split('T')[0] <= n.toISOString().split('T')[0] 
					&& this.pass_e.toISOString().split('T')[0] <= d_max.toISOString().split('T')[0] 
					&& this.pass_e.toISOString().split('T')[0] >= de.toISOString().split('T')[0] 
	...
	template : `
<v-dialog v-model="dialog" class="custom-dialog flight-dialog dialog-creare-pasager">
	<template v-slot:default="{ isActive }">
		<v-card class="align-self-center" style="max-width: min(95vw, 630px);width:630px">
		<v-card-title>Adauga pasager nou</v-card-title>
        ...
        <v-form ref="form" v-model="valid">
          ...
          <v-text-field ref="firstname"
            class="rounded-theme"
              ...
              required
            ></v-text-field>
          <v-icon icon="mdi-arrow-expand-vertical" class="text-primary" theme="light" style="position: absolute;left: 0;margin-top: -22px;" @click="() => {var d = '' + this.firstname; this.firstname = this.lastname; this.lastname = d;}"/>
          <v-text-field ref="lastname"
			...
			<DatePickerSelect v-model="dob" :age="age_range" style="width: 360px;" :rules="dobRules" :reference-date="referenceDate" return-type="Date" hide-details-obs  validate-on="lazy input"></DatePickerSelect>
			
			<?php /*<IOSDatepicker :start-date="date || max_date" :preview-format="a => formatDate(a)" v-model="dob" month-name-format="long" locale="ro" :year-range="year_range" inline :min-date="min_date" :max-date="max_date" calendar-class-name="v-theme--light" class="w-100 px-4" :enable-time-picker="false" :rules="dobRules" @click="unfocusElem"></IOSDatepicker> */ ?>
            <div>
			
			<v-list-item-title class="pa-0 text-h5 d-none">Data referinta: <span v-html="formatDate(referenceDateEnd)"></span> </v-list-item-title>
			...
			<v-radio-group inline color="primary" v-model="doctype" :rules="docTypeRules" class="mt-2 mb-3">
				...
				<v-radio label="CI" value="1" :disabled="passport_required"></v-radio>
				<?php /*  */ ?>
			...
			*/ ?>
			...
			<DatePickerSelect v-model="ci_s" :age="[0, 59]" style="width: 360px;" :rules="docDateRules" :reference-date="today" return-type="Date" hide-details-obs  validate-on="lazy input"></DatePickerSelect>
			
			<?php /*<IOSDatepicker :start-date="ci_s || today" :preview-format="a => formatDate(a)" v-model="ci_s" month-name-format="long" locale="ro" :year-range="[min_date.getFullYear(), today.getFullYear()]" inline :min-date="min_date" :max-date="today" calendar-class-name="v-theme--light" class="w-100 px-4" :enable-time-picker="false" :rules="docDateRules"></IOSDatepicker> */ ?>
			<h5>Data EXPIRARE carte de identitate:</h5>
			
			<DatePickerSelect v-model="ci_e" :age="[0, 59]" style="width: 360px;" :rules="docDateRules" :reference-date="exp_date" return-type="Date" hide-details-obs  validate-on="lazy input"></DatePickerSelect>
			
			<?php /*<IOSDatepicker :start-date="ci_e || today" :preview-format="a => formatDate(a)" v-model="ci_e" month-name-format="long" locale="ro" :year-range="[min_date.getFullYear(), exp_date.getFullYear()]" inline :min-date="min_date" :max-date="exp_date" calendar-class-name="v-theme--light" class="w-100 px-4" :enable-time-picker="false" :rules="docDateRules"></IOSDatepicker>*/ ?>
			</template>
			...
			<DatePickerSelect v-model="pass_s" :age="[0, 59]" style="width: 360px;" :rules="docDateRules" :reference-date="today" return-type="Date" hide-details-obs  validate-on="lazy input"></DatePickerSelect>
			<?php /*<IOSDatepicker :start-date="pass_s || today" :preview-format="a => formatDate(a)" v-model="pass_s" month-name-format="long" locale="ro" :year-range="[min_date.getFullYear(), today.getFullYear()]" inline :min-date="min_date" :max-date="today" calendar-class-name="v-theme--light" class="w-100 px-4" :enable-time-picker="false" :rules="docDateRules"></IOSDatepicker> */ ?>
			<h5>Data EXPIRARE pasaport:</h5>
			<DatePickerSelect v-model="pass_e" :age="[0, 59]" style="width: 360px;" :rules="docDateRules" :reference-date="exp_date" return-type="Date" hide-details-obs  validate-on="lazy input"></DatePickerSelect>
			<?php /* <IOSDatepicker :start-date="pass_e || today" :preview-format="a => formatDate(a)" v-model="pass_e" month-name-format="long" locale="ro" :year-range="[min_date.getFullYear(), exp_date.getFullYear()]" inline :min-date="min_date" :max-date="exp_date" calendar-class-name="v-theme--light" class="w-100 px-4" :enable-time-picker="false" :rules="docDateRules"></IOSDatepicker> */ ?>
			</template>
            </div>
	  ...
	  <v-card-actions>
		...
		<v-btn class="text-none font-weight-normal cancel-button" variant="outlined" size="large" rounded="theme" @click="dialog = false"><v-icon icon="mdi-arrow-left"></v-icon> Inapoi</v-btn>
        <v-btn class="d-flex text-capitalize font-weight-normal" variant="outlined" style="flex:1;" size="large" :color="true ? 'primary' : 'secondary'" rounded="theme" @click="validateAndSave().then((v) => v && (dialog = false))" v-html="'Confirma'"></v-btn>
	  </v-card-actions>
	  </v-card>
	</template>
</v-dialog>
  ...
  computed: {
	...
	ciRules: {
		get() { 
			return Object.freeze([
				...
				v => {
					if(!(this.doctype == '1') || (v || '').trim() === '') return true; 
					if(this.nationality == 'RO'){
						return !/^[0-9]{13}$/.test((v || '').trim()) && 'CI trebuie sa contina 13 cifre' || this.validCNP(v) || "CNP Invalid";
					}
					return /^[0-9]{9,20}$/.test((v || '').trim()) || 'CI trebuie sa contina 9-20 cifre';
				},
			])
		}
	},
	...
	ciSerieRules: {
		get() { 
			return Object.freeze([
				v => !/[^a-zA-Z]/.test((v || '').trim()) || 'Seria documentului trebuie sa contina doar litere',
				v => {
					return true;
					if(!(this.doctype == '1')) return true; 
					return !!(v || '').trim().length || 'Seria documentui este obligatorie';
				}
			])
		}
	},
  ...
  watch:{
    ...
    'type': {
      handler(newValue, oldValue){
		...
		this.age_range = [min_years, max_years]
