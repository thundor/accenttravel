...
foreach($index_ids as $index_id){
	...
	$cache_key = 'newux/trip/flight/inspect/' . md5(json_encode($index_id));
	...
	if(!isset($response)){
		...
		$r = $this->Flights_model->api->apiCall('index.php/en/dynamic-package/sid/' . $index_id);
		$r->status = 2;
...
// $rs[] = $this->Flights_model->api->apiCall('index.php/en/dynamic-package/inspect/' . $container_id);
echo json_encode($rs);