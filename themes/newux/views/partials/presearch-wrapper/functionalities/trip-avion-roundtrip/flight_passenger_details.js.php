...
export default {
  ...
  emits: ['edit', 'result', 'save', 'research', 'assigned_passengers'],
  props: {
      modelValue: {
          type: Object,
          default: () => ({}),
  ...
  components : {
    ...
    <div class="rounded-theme"> 
      ...
      */ ?>
      ...
      <v-btn-toggle v-if="all_routes.length > 1"
        ...
        multiple
        class="flex-wrap mb-4 ga-3"
        theme="light"
      >
        <v-btn v-for="route in all_routes" :value="route" :active="false" class="text-none min-content" :color="selected_routes && (-1 !== selected_routes.indexOf(route)) ? 'primary' : 'default'">
      ...
      */ ?>
      ...
      <template v-slot:default-header="prop">
        <div @click="(!(prop.node.children && prop.node.children.length) && (prop.ticked = !prop.ticked))" class="v-theme--light w-100 d-flex justify-space-between">
        <span class="">
          {{ prop.node.label }}
        </span>
    ...
    watch: {
      ...
      'selected_routes':{
        handler(newValue, oldValue){
			if(JSON.stringify(newValue) === JSON.stringify(oldValue)) return;
          // console.warn('shouldReload', newValue, oldValue);
          this.reloadNodes();
        },
        // immediate: true,
      },
    ...
    mounted: function() {
      ...
      data: () => ({
        ...
        <template v-if="segments && segments.length">
          <div class="w-100 d-flex text-none align-center ga-3">
	...
	template : `
<template v-if="flight_data">
  <FormLegend title="Detalii pasageri">
	<template v-slot:subtitle>
	<p v-if="(secured || identification_required) && !passport_required">Pentru acest zbor trebuie completate corect informatiile referitoare la documentul de identificare (CI / Pasaport) pentru fiecare pasager</p>
	<p v-if="(secured || identification_required) && passport_required">Pentru acest zbor trebuie completate corect informatiile referitoare la Pasaport pentru fiecare pasager</p>
	</template>
  </FormLegend>
  <div class="passenger-details v-theme--light">
  <div v-for="pax_fare in [{PTC:'ALL'}]" v-if="optional_services['ALL,']" :set="(pax_split_index=undefined)" class="bg-background rounded-theme mb-4">
	<div class="bg-background2 pa-4 d-flex justify-start align-center">
          <span class="ml-4">Toti pasagerii</span>
        </div>
	<div class="pa-4 pt-0">
    <v-dialog v-if="optional_services[[pax_fare.PTC, pax_split_index]] && optional_services[[pax_fare.PTC, pax_split_index].join(',')].filter((v) => !getObjectDotPathValue(v,'Included', false)).length" class="custom-dialog flight-dialog dialog-servicii-incluse dialog-servicii-incluse-toti">
        <template v-slot:activator="{ props }">
			<div class="d-flex mt-4 align-center">
			  <v-icon :icon="'mdi-bag-suitcase'" class="mr-4 flex-grow-0"></v-icon>
			  <v-btn v-bind="props" class="rounded-theme flex-fill d-flex py-4 modal-button text-none align-center justify-space-between pa-4" variant="outlined">
				<div class="d-flex flex-wrap align-center">
				  <span class="me-2">Alege servicii aditionale pentru toti passagerii</span>
				</div>
				<template v-slot:append v-if="optionalServicesPriceFor[[pax_fare.PTC, pax_split_index]]">
				  <strong class="text-pre" v-html="format_price(optionalServicesPriceFor[[pax_fare.PTC, pax_split_index]], flight_data.Currency)"></strong>
				</template>
			  </v-btn>
			  
			</div>
		  
		  <div class="d-flex w-100 justify-space-between mb-2" v-for="option in ((result || {}).paidOptions || []).filter(o => (o.Target == pax_fare.PTC))">
				<div>
					<div>{{ getObjectDotPathValue(option,'Service.Name') }} ({{ getObjectDotPathValue(option,'Service.CategoryName') }}): <b>{{ getObjectDotPathValue(option,'Option.Description.0') }}</b> ({{ getObjectDotPathValue(option,'Route.From') + '-' + getObjectDotPathValue(option,'Route.To') }})</div>
					<div>{{ [...(option.Description || [])].join('; ') }}</div>
				</div>
				<span class="text-primary">{{ format_price(getObjectDotPathValue(option,'Option.Price.Amount',0), getObjectDotPathValue(option,'Option.Price.Currency', flight_data.Currency), 1) }}</span>
			  </div>
		  
        </template>
		<template v-slot:default="{ isActive }">
		<v-card class="align-self-center" style="max-width: min(95vw, 630px);width:630px">
		<v-card-title>Servicii aditionale</v-card-title>
		<v-card-text class="max-height overflow-y-auto px-0">
        <v-list lines="two" subheader theme="light" class="" rounded="theme">
          ...
          <div class="px-4" v-for="(optionalService, serviceIndex) in optional_services[[pax_fare.PTC, pax_split_index]].filter((v) => !getObjectDotPathValue(v,'Included', false))">
            ...
            <div v-if="getObjectDotPathValue(optionalService,'Options.Option')" class="v-theme--light">
              <template v-for="(option,optionIndex) in optionalService.Options.Option">
                <v-checkbox theme="light" color='primary' multiple hide-details density="compact" :value="[optionalService._index,optionIndex]" v-model="optionalServices" class="align-start optional-service-check">
                  <template v-slot:label>
                    <small class="color-light-light" style="word-break: break-all;">{{ [...(option.Description || [])].join('; ') }}</small>
                  </template>
	  ...
	  <v-card-actions>
		<v-spacer></v-spacer>
          <v-btn @click="isActive.value = false" class="text-none font-weight-normal flex-grow-1" size="large" variant="outlined" rounded="theme">Confirma<template v-if="optionalServicesPriceFor[[pax_fare.PTC, pax_split_index]]"> ({{ format_price(optionalServicesPriceFor[[pax_fare.PTC, pax_split_index]], flight_data.Currency) }})</template></v-btn>
	  </v-card-actions>
	  </v-card>
	</template>
    </v-dialog>
	<div class="servicii-incluse bg-background2 pa-4 rounded-theme mb-4 text-left" v-if="optional_services[[pax_fare.PTC, pax_split_index]] && optional_services[[pax_fare.PTC, pax_split_index]].filter((v) => getObjectDotPathValue(v,'Included', false)).length">
		...
		<div v-for="(optionalService, serviceIndex) in optional_services[[pax_fare.PTC, pax_split_index]].filter((v) => getObjectDotPathValue(v,'Included', false))">
		  <div class="serviciu-inclus d-flex align-center justify-space-between">{{ capitalizeWords(getObjectDotPathValue(optionalService,'Service.Name'),1) }} ({{ capitalizeWords(getObjectDotPathValue(optionalService,'Service.CategoryName'),1) }}) ({{ getObjectDotPathValue(optionalService,'Route.From') + '-' + getObjectDotPathValue(optionalService,'Route.To') }})
		  </div>
		</div>
	</div>
	</div>
  ...
  <template v-for="pax_fare in (flight_data?.FareDetails?.PaxFare || [])">
    <template v-for="(pax_split_item, pax_split_index) in parseInt(pax_fare.Count)">
      <div class="bg-background rounded-theme mb-4" style="overflow:hidden;">
        ...
        <div class="pa-4 pt-0">
			<div class="d-flex flex-fill flex-wrap justify-space-between">
				<div class="flex-fill order-lg-1 d-flex ga-2 flex-column">
					<component :is="loadViewAsync('partials/presearch-wrapper/functionalities/<?php echo basename(dirname($a)); ?>/flight_details_passenger')" :referenceDate="referenceDate" :referenceDateEnd="referenceDateEnd" :identification_required="identification_required" :passport_required="passport_required" v-on:remove_passenger="(p) => removePassenger(p)" v-on:assign_passenger="(p) => addPassenger(p, pax_fare.PTC, pax_split_index)" :secured="secured" :type="pax_fare.PTC" v-on:open_create_passenger="(p) => openCreatePassengerModel(p, pax_split_index, pax_fare.PTC)" :passengers="passengers" :assigned_passengers="assigned_passengers" :assigned_passenger="assigned_passengers[[pax_fare.PTC,pax_split_index].join(',')]"></component>
					
					
          <v-dialog v-if="optional_services[[pax_fare.PTC, pax_split_index]] && optional_services[[pax_fare.PTC, pax_split_index]].filter((v) => !getObjectDotPathValue(v,'Included', false)).length" class="custom-dialog flight-dialog dialog-servicii-incluse dialog-servicii-incluse-particular">
            <template v-slot:activator="{ props }">
				<div class="d-flex mt-4 align-center">
				  <v-icon :icon="'mdi-bag-suitcase'" class="mr-4 flex-grow-0"></v-icon>
				  <v-btn v-bind="props" class="rounded-theme flex-fill d-flex py-4 modal-button text-none align-center justify-space-between pa-4" variant="outlined">
			  ...
			  <div class="d-flex w-100 justify-space-between mb-2" v-for="option in ((result || {}).paidOptions || []).filter(o => (o.Target == pax_fare.PTC) && o.PassengerIndex == pax_split_index)">
				<div>
					<div>{{ getObjectDotPathValue(option,'Service.Name') }} ({{ getObjectDotPathValue(option,'Service.CategoryName') }}): <b>{{ getObjectDotPathValue(option,'Option.Description.0') }}</b> ({{ getObjectDotPathValue(option,'Route.From') + '-' + getObjectDotPathValue(option,'Route.To') }})</div>
					<div>{{ [...(option.Description || [])].join('; ') }}</div>
				</div>
				<span class="text-primary">{{ format_price(getObjectDotPathValue(option,'Option.Price.Amount',0), getObjectDotPathValue(option,'Option.Price.Currency'), 1) }}</span>
			  </div>
            </template>
			<template v-slot:default="{ isActive }">
		<v-card class="align-self-center" style="max-width: min(95vw, 630px);width:630px">
	  ...
	  <v-card-actions>
		<v-spacer></v-spacer>
          ...
          <template v-if="'INF' != pax_fare.PTC" v-for="(route, routeIndex) in flight_data.Routes">
            <div class="d-flex mt-4 align-center">
              <v-icon :icon="!routeIndex ? 'mdi-airplane-takeoff' : 'mdi-airplane-takeoff mdi-flip-h'" class="mr-4 flex-grow-0"></v-icon>
              <v-btn @click="openSeatAssignment(pax_split_index, pax_fare.PTC, assigned_passengers[[pax_fare.PTC,pax_split_index]], routeIndex)" class="rounded-theme flex-grow-1 justify-space-between d-flex py-4 text-none modal-button" append-icon="mdi-chevron-right" variant="outlined" style="width:calc(100% - 40px);">
			  ...
			  <template v-if="seats[[pax_fare.PTC, pax_split_index]] && seats[[pax_fare.PTC, pax_split_index]][routeIndex]">
				<div class="locuri-alese ps-9">
				<template v-for="(seat,segmentIndex) in seats[[pax_fare.PTC, pax_split_index]][routeIndex]">
				  <template v-if="seat">
					<div class="d-flex mr-4 w-100 justify-space-between" v-for="s in [this.flight_data.Routes[routeIndex].Segment[segmentIndex]]">
					  {{ s.Origin.Airport.City }} - {{ s.Destination.Airport.City }}:
					  <template v-if="seat=='A'">
						<span class="ml-1">Culoar</span>
						<span class="ml-1 text-primary">{{ format_price(0, flight_data.Currency, 1) }}</span>
					  </template>
					  <template v-else-if="seat=='W'">
						<span class="ml-1">Fereastra</span>
						<span class="ml-1 text-primary">{{ format_price(0, flight_data.Currency, 1) }}</span>
					  </template>
					  <template v-else v-for="s in [SeatMaps[[routeIndex, segmentIndex]]]">
					  <template v-if="s" v-for="r in [s.Rows.Row[seat.split(',')[0]].Seat[seat.split(',')[1]]]">
						<span class="ml-1">{{ ('' + r.Number + r.Code) }}</span>
						<span class="ml-1 text-primary">{{ (r.ChargeTypeReference ? (c = s.ChargeList.ChargeType.find((v) => v.Reference == r.ChargeTypeReference), c && format_price(c.Price.Amount, c.Price.Currency, 1)) : format_price(0, flight_data.Currency, 1)) }}</span>
					  </template>
					  </template>
					</div>
				  </template>
				...
				<div class="flex-fill order-lg-0">
...
<component :is="loadViewAsync('partials/presearch-wrapper/functionalities/<?php echo basename(dirname($a)); ?>/flight_details_create_passenger')" :referenceDate="referenceDate" :referenceDateEnd="referenceDateEnd" :identification_required="identification_required" :passport_required="passport_required" :secured="secured" ref="create_passenger_modal" v-on:save="(p, ptc, cnt) => this.addPassenger(p, ptc, cnt)"></component>
<component :is="loadViewAsync('partials/presearch-wrapper/functionalities/<?php echo basename(dirname($a)); ?>/flight_details_seat_assignment')" :flight_data="flight_data" ref="assign_seat_modal" :inspection="inspection" :data="data" v-on:save="( ptc, cnt, routeIndex, segmentIndex, selectedSeat, defaultSeat) => this.saveSeatAssignment(ptc, cnt, routeIndex, segmentIndex, selectedSeat, defaultSeat)" v-on:seat_map="setSeatMap" v-on:research="(h, t) => $emit('research', h, t)"></component>
</div>
</template>
	`,
  methods: {
    ...
    saveSeatAssignment(ptc, cnt, routeIndex, segmentIndex, selectedSeat){
    ...
    savePassengers(){
      ...
      saveStorage('newux.flight.passengers',{
    ...
    recalculate(){
      ...
      for(var i in this.optionalServices || {}){
        ...
        options_price += parseFloat((optionalServiceOption.Price || {}).Amount || 0);
      ...
      serviceData.passenger = (getObjectDotPathValue(this.flight_data || {}, 'FareDetails.PaxFare') || []).reduce((a, pf) => (a[pf.PTC] = [], [...Array(parseInt(pf.Count)).keys()].forEach(k => (a[pf.PTC].push({
		...
		... (this.secured || this.identification_required ? {
			...
			idDocNumber: ((this.assigned_passengers[[pf.PTC, k]] || {}).doctype || '0') == '0' ? ((this.assigned_passengers[[pf.PTC, k]] || {}).pass) : ((this.assigned_passengers[[pf.PTC, k]] || {}).ci_nr),
    ...
    openSeatAssignment(pax_split_index, ptc, assigned_passenger, routeIndex){
    ...
    openCreatePassengerModel(p, pax_split_index, ptc){
      ...
      this.$refs.create_passenger_modal.pass_c = p.pass_c || this.$refs.create_passenger_modal.nationality;
      ...
      this.$refs.create_passenger_modal.ci_nr = p.ci_nr || '';
      this.$refs.create_passenger_modal.ci_serie = p.ci_serie || '';
  ...
  beforeMount: function() {
    ...
    var storageItem = getStorage('newux.flight.passengers','', {}, {}, {});
    ...
    if(savedAccount){
      var profile, personal_data, profile_ids;
      if(profile = getObjectDotPathValue(savedAccount,'profile') || {}){
		personal_data = getObjectDotPathValue(profile,'personal_data') || {};
		profile_ids = getObjectDotPathValue(profile,'id') || [];
        var citizenship = (personal_data.citizenship || '').trim().replace(/\s+/g, ' ');
		if(citizenship.toLowerCase() == 'romana'){
			citizenship = 'Romania';
		}
        var country;
		...
		if(/^(0[1-9]|[1-2][0-9]|3[01])\.(0[1-9]|1[0-2])\.([0-9]{4})$/.test(personal_data.birth_date || '')){
			personal_data.birth_date = personal_data.birth_date.split(/\./).reverse().join('-')
		}
        var p = {
          title: 'mr',
        ...
        if(profile_ids && profile_ids.length){
			profile_ids.forEach((docdata) => {
				if(docdata.type == 'CI' && !p.ci){
					p.ci = docdata.PersonalIdentificationCode || '';
					p.ci_nr = docdata.nr || '';
					p.ci_serie = docdata.series || '';
					p.ci_s = docdata.IssuedAt || undefined;
					p.ci_e = docdata.ExpiresOn || undefined;
					if(undefined == p.doctype) p.doctype = '1';
				}
				if(docdata.type == 'PASS' && !p.pass){
					p.pass = docdata.nr || '';
					p.pass_s = docdata.IssuedAt || undefined;
					p.pass_e = docdata.ExpiresOn || undefined;
					if(undefined == p.doctype || '1' == p.doctype) p.doctype = '0';
					var citizenship = (docdata.Citizenship || '').trim().replace(/\s+/g, ' ');
					if(citizenship.toLowerCase() == 'romana'){
						citizenship = 'Romania';
					}
					var country;
      ...
      if((associated_persons = getObjectDotPathValue(savedAccount,'associated_persons')) && (associated_persons = associated_persons.flat(1)).length){
        associated_persons.forEach((profile) => {
			var personal_data = getObjectDotPathValue(profile,'personal_data') || {};
          var n = ('' + (personal_data.last_name || '') + ' ' +  (personal_data.first_name || '')).trim();
          if(!n.length) return;
		  var fn = (personal_data.last_name || '').trim().replace(/\s+/g, ' ');
		  var ln = (personal_data.first_name || '').trim().replace(/\s+/g, ' ');
		  var e = (personal_data.email || '').trim().replace(/\s+/g, ' ');
		  var citizenship = (personal_data.citizenship || '').trim().replace(/\s+/g, ' ');
		  if(citizenship.toLowerCase() == 'romana'){
				citizenship = 'Romania';
			}
          ...
          var p = {
            title: 'mr',
            firstname: e ? fn : capitalizeWords((n.replace(/\s.*/, '') || '').trim().replace(/\s+/g, ' '))  ,
            lastname: e ? ln : capitalizeWords((n.replace(/.*?(\s|$)/, '') || '').trim().replace(/\s+/g, ' ')),
			birth_date: (personal_data.birth_date || '').trim().replace(/\s+/g, ' '),
			nationality: country && country.value || 'RO',
          };
          // p.firstname += ' ' + JSON.stringify(personal_data);
		  profile_ids = getObjectDotPathValue(profile,'id') || [];
		  if(profile_ids && profile_ids.length){
			profile_ids.forEach((docdata) => {
				if(docdata.type == 'CI' && !p.ci){
  ...
  watch:{
    'flight_data': {
      handler(newValue, oldValue){
		...
		if(research_result){
			...
			this.inspection.checking.options = true;
			var seatcheck = new Promise((a) => a(false));
			seatcheck.then(errors => {
				// console.warn('seat errors', errors);
				var optionalServices = [];
				var option_error = false;
				if(research_result?.paidOptions && research_result?.paidOptions.length){
					research_result?.paidOptions.forEach(ro => {
						var selected_option;
						(this.optional_services?.[[ro.Target, ro.PassengerIndex]] || []).find((s, serviceIndex) => {
							var found_s = JSON.stringify(s?.Service) === JSON.stringify(ro?.Service);
							if(found_s){
								// console.log('found_s', s);
							}
							return found_s
								&& (s?.Options?.Option || []).find((o, optionIndex) => {
									return o.Code == ro?.Option?.Code
										 && (selected_option = [serviceIndex, optionIndex]) || true
								});
						});
						if(!selected_option){
							option_error = true;
							console.warn("Unable to find option for", [ro.Target, ro.PassengerIndex], ro?.Option.Description);
						} else {
							optionalServices.push(selected_option.join(','));
						}
						// this.optionalServices
					})
				}
				if(optionalServices.length){
					...
					// console.warn("TO SELECT OPTIONS", JSON.stringify(this.optionalServices));
				} else {
					console.warn("NO SELECT OPTIONS");
				}
			}).finally(() => {
				if(this.inspection?.checking)
				this.inspection.checking.options = false;
			})
		}
    ...
    'optionalServices': {
      handler(newValue, oldValue){
        // console.warn('optionalServices watch recalculate', newValue),
        this.recalculate();
      },
      immediate: true,
      deep: true,
    ...
    'assigned_passengers': {
      handler(newValue, oldValue){
        ...
        this.recalculate();
		if(newValue){
			this.$emit('assigned_passengers', newValue);
		}
      },
      immediate: true,
      deep: true,
    ...
    'set_assigned_passengers': {
      handler(newValue, oldValue){
		  if(newValue){
			  if(JSON.stringify(newValue) !== JSON.stringify(this.assigned_passengers || {})){
				this.assigned_passengers = newValue;
			  }
		  }
      },
    },
