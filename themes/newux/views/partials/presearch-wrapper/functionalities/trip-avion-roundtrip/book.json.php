...
$flight2 = $post['flight2'] ?? array();
if($flight2){
	$_POST['flight2']['expectedFlightPrice'] = number_format(($flight2['expected_price'] ?? ($post['price'] ?? null)),2,'.','') . ($post['currency'] ?? null);
	$_POST['flight2']['itinerary_code'] = $flight2['itinerary_code'] ?? null;
	$_POST['flight2']['flight_code'] = $flight2['code'] ?? null;
	$_POST['flight2']['upsellCode'] = $flight2['upsellCode'] ?? null;

	$_POST['flight2']['paidSeats'] = $flight2['paidSeats'] ?? null;
	$_POST['flight2']['optionalServices'] = $flight2['optionalServices'] ?? null;

	$_POST['flight2']['passenger'] = null; // TODO
	$_POST['flight2']['preferredSeats'] = null; // TODO

	foreach($flight2['passenger'] as $ptc => $ptc_passengers){
		foreach($ptc_passengers as $ptc_passenger_index => $flight_passenger){
		  
			$_POST['flight2']['passenger']['title'][] = $flight_passenger['title'] ?? 'mr';
			$_POST['flight2']['passenger']['birth_date'][] = date("d.m.Y", strtotime($flight_passenger['birthDate'] ?? date('Y-m-d')));
			$_POST['flight2']['passenger']['firstname'][] = $flight_passenger['firstName'] ?? '';
			$_POST['flight2']['passenger']['lastname'][] = $flight_passenger['lastName'] ?? '';
			$_POST['flight2']['passenger']['country'][] = $flight_passenger['country'] ?? '';

			if(isset($flight_passenger['details']) && is_array($flight_passenger['details'])){
				$_POST['flight2']['preferredSeats'][$ptc][$ptc_passenger_index]['details'] = $flight_passenger['details'];
			}
		}
	}
}
...
// dd($this->Trip_model->get_api()->calls);
ob_start(); /* ?>
