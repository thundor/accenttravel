...
export default {
	...
	data: () => {
		return {
			...
			filter_component: '',
			results_component: '',
			offer_component: 'partials/presearch-wrapper/functionalities/<?php echo basename(dirname($a)); ?>/offer-roundtrip',
	...
	<v-btn-toggle mandatory
        v-model="wind"
        class="flight-flight2-toggle flex-wrap mb-4 ga-3"
        theme="light"
      >
        <v-btn :value="0">
          <small>Zbor Tur</small>
        </v-btn>
        <v-btn :value="1">
          <small>Zbor Retur</small>
        </v-btn>
	</v-btn-toggle>
	<v-window v-model="wind" id="flights-flights2-window">
		<v-window-item eager>
			<template v-if="flight2" v-for="result in [flight2]">
				<FormLegend title="Zbor Retur"></FormLegend>
				<?php ob_start(); ?>
				<v-card class="mb-8 border-lg border-primary detalii-zbor-roundtrip-ales detalii-ales">
				...
				<?php $selected_flight_details = ob_get_flush(); ?>
			</template>
			<FormLegend title="Alege zbor Tur:"></FormLegend>
			<hr />
			<br />
			<component ref="flight_search" :is="loadViewAsync('partials/presearch-wrapper/functionalities/<?php echo basename(dirname($a)); ?>/search-flights')" select_first :data="data" :key_path="key_path" :show_breadcrumbs="false" v-on:offer="(h,l) => (flight = h, flights = l)" :extra_price="parseFloat(((flight2 || {}).PriceDetail || {}).Amount || 0)"></component>
		</v-window-item>
		
		<v-window-item eager>
			<template v-if="flight" v-for="result in [flight]">
				<FormLegend title="Zbor Tur"></FormLegend>
				<?php echo $selected_flight_details; ?>
			</template>
			<FormLegend title="Alege zbor Retur:"></FormLegend>
			<hr />
			<br />
			<component ref="flight2_search" :is="loadViewAsync('partials/presearch-wrapper/functionalities/<?php echo basename(dirname($a)); ?>/search-flights2')" select_first :data="data" :key_path="key_path" :show_breadcrumbs="false" v-on:offer="(h,l) => (flight2 = h, flights2 = l)" :extra_price="parseFloat(((flight || {}).PriceDetail || {}).Amount || 0)"></component>
		</v-window-item>
	</v-window>
	</v-container>
	<teleport to="#pos-stick-b-t" v-if="(1 == data.step)">
		<div v-if="flight && flight2" class="d-flex w-100 justify-space-between py-1 align-center ga-2">
			<v-btn class="buton-sumar" variant="outlined" size="large" @click="wind = wind ? 0 : 1"><span v-if="!wind">Alege alt zbor retur</span>
					<span v-if="wind">Alege alt zbor tur</span>
			</v-btn>
			<div class="d-none d-md-flex flex-column ga-1">
				<div class="d-flex w-100 justify-space-between ga-2"><span>Bilete avion Tur</span><strong>{{ format_price((getObjectDotPathValue(flight,'PriceDetail.Amount',0)), getObjectDotPathValue(flight,'Currency'), 1) }}</strong></div>
			</div>
			<div class="d-none d-md-flex flex-column ga-1">
				<div class="d-flex w-100 justify-space-between ga-2"><span>Bilete avion Retur</span><strong>{{ format_price((getObjectDotPathValue(flight2,'PriceDetail.Amount',0)), getObjectDotPathValue(flight2,'Currency'), 1) }}</strong></div>
			</div>
			<v-btn
				class="button-alege justify-self-end"
				@click="(activate_next_step++, setOffer(flight2))"
				size="large"
				variant="outlined"
			>
				<span>
					<span>Configurare Roundtrip</span> <span v-text="'(' + format_price(parseFloat(flight.PriceDetail.Amount) + parseFloat(flight2.PriceDetail.Amount), flight.Currency) + ')'"></span>
				</span>
			</v-btn>
		</div>
	...
	<teleport to="#search-wrapper-item-content" v-if="(2 <= data.step) && flight2 && activate_next_step">
		<component :is="loadViewAsync(offer_component)" :offer="flight" :inspection="flights" :flight2="flight2" :flight2_inspection="flights2" :searching="searching" :prepend_breadcrumbs="breadcrumbs" v-on:hash="researchHash" v-on:research="research"
	...
	methods: {
		...
		setOffer(offer){
			...
			setTimeout(() => (this.flight2 = offer, this.$emit('set-value', {step:2})), 0)
		},
	},
	computed: {
		breadcrumbs() {
