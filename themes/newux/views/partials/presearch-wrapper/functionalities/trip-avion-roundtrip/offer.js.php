...
export default {
	...
	inject: ['search'],
	emits: ['offer','set-value','research', 'assigned_passengers'],
	components:{
      ...
      handler(newValue, oldValue){
		...
		'ancilleryOrFlight': {
			handler: function(nv,ov){
				...
				this.flight.ancilleryOrFlight = nv;
			},
			immediate: true
		},
		'result': {
	...
	<v-container class="bg-background" v-if="flight" id="wrapper-oferta-zboruri">
		...
		<?php /* 
		<div v-if="flight2" class="">
			<div class="d-flex w-100 justify-space-between mb-2"><span>flight2</span><div>{{ JSON.stringify(flight2) }}</div></div>
		</div> 
		*/ ?>
		<div class="d-flex justify-space-between w-100 flight-details-wrapper flex-column">
			...
			<component v-if="ancilleryOrFlight" :is="loadViewAsync('partials/presearch-wrapper/functionalities/<?php echo basename(dirname($a)); ?>/flight_passenger_details')" v-on:assigned_passengers="(v) => (this.assigned_passengers = v)" :set_assigned_passengers="assigned_passengers" ref="passenger_details" :passport_required="passport_required" :flight_data="ancilleryOrFlight" :data="data" :inspection="inspection" v-on:result="(r) => (this.result = r)" v-on:save="(chosen_index = (chosen_index || 0)+1, console.warn('setting step', 3), $.emit('set-value', {step: 3}))" v-on:research="(h, t) => $emit('research', h, t)"></component>
			...
			<teleport to="#pos-stick-b-t" v-if="!no_next && (2 == search_wrapper_step)">
				<div class="d-flex w-100 justify-space-between py-1"  v-if="ancilleryOrFlight && (!flight.loadingUpsells || !Object.values(flight.loadingUpsells).length)" v-for="flight in [{...ancilleryOrFlight, result:(result || {})}]">
					...
					<div class="d-none d-md-flex flex-column ga-1">
						<div class="d-flex w-100 justify-space-between ga-2"><span>Bilete</span><strong>{{ format_price((getObjectDotPathValue(flight,'FareDetails.FullFare',0) + getObjectDotPathValue(flight2,'FareDetails.FullFare',0)), getObjectDotPathValue(flight,'Currency'), 1) }}</strong></div>
						<div class="d-flex w-100 justify-space-between ga-2"><span>Taxa de serviciu</span><strong>{{ format_price((getObjectDotPathValue(flight,'FareDetails.ServiceFee',0) + getObjectDotPathValue(flight2,'FareDetails.ServiceFee',0)), getObjectDotPathValue(flight,'FareDetails.Currency'), 1) }}</strong></div>
					</div>
					<div class="d-none d-md-flex flex-column ga-1">
						<div class="d-flex w-100 justify-space-between ga-2"><span>Locuri preferentiale</span><strong>{{ format_price((getObjectDotPathValue(flight,'result.seatsPrice',0) + getObjectDotPathValue(flight2,'result.seatsPrice',0)), getObjectDotPathValue(flight,'Currency'), 1) }}</strong></div>
						<div class="d-flex w-100 justify-space-between ga-2"><span>Servicii Extra</span><strong>{{ format_price((getObjectDotPathValue(flight,'result.optionsPrice',0) + getObjectDotPathValue(flight2,'result.optionsPrice',0)), getObjectDotPathValue(flight,'Currency'), 1) }}</strong></div>
					</div>	
		...
		<teleport to="#search-wrapper-item-checkout" v-if="!no_next && result && undefined !== chosen_index && ancilleryOrFlight && flight2">
			<component :is="loadViewAsync(set_checkout_component || checkout_component)" :search_data="search_data" :result="{Flight: ancilleryOrFlight && {...ancilleryOrFlight, result:result} || undefined, Flights: inspection, Hotel: hotel, Hotels: hotel_inspection, Flight2: flight2, Flights2: flight2_inspection}" :prepend_breadcrumbs="breadcrumbs" :data="data" :search_wrapper_step="search_wrapper_step" v-on:set-value="(v) => $emit('set-value', v)" v-on:research="(h, t) => $emit('research', h, t)"></component>
		</teleport>
	</v-container>
	`,
}
