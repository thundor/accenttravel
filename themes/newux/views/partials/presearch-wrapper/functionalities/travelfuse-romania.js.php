...
export default {
	...
	methods: {
		clickSelected: function(who, what){
			...
			var p = this.functionalities.indexOf(a);
			if(-1 !== p && a !== 'travellers'){
				var i = document.getElementsByClassName('menu-<?php echo basename(__FILE__, '.js.php'); ?>/' + this.functionalities[1 + p] + '-wrapper'); 
				if(i && i[0]){
					who.opened = false;
					i[0].click()
				}
	...
	mounted() {
		if(this.content_type == 'default'){
			...
			if(!isNaN(step) && step > 0){
				setTimeout(() => {
					this.$emit('set-value',{'<?php echo basename(__FILE__, '.js.php'); ?>/submit': true});
				}, 1000)
	...
	watch: {
		'data.<?php echo basename(__FILE__, '.js.php'); ?>/submit': {
			handler: function(nv,ov){
				...
				var to_save_data = this.functionalities.filter(f => !/submit$/.test(f)).reduce((c,i) => {
		...
		<component :is="loadViewAsync('partials/presearch-wrapper/functionality')" :active_menu="active_menu" :data="data" v-on:set-value="(v) => $.emit('set-value', v)" v-on:click-selected="clickSelected" :search_wrapper_step="search_wrapper_step" :disabled="disabled">
			<template v-slot:default>
				...
				<teleport :to="!search_wrapper_step ? '#<?php echo basename(__FILE__, '.js.php'); ?>-menu' : '#search-wrapper-step-1'">
					<component :is="loadViewAsync('partials/presearch-wrapper')" :functionalities="functionalities.map(v => key + '/' + v)" :data="data" v-on:set-value="(v) => $.emit('set-value', v)" v-on:click-selected="clickSelected" :search_wrapper_step="search_wrapper_step" :key_path="(key_path ? key_path + '.' : '') + key">
