...
$in_city_breaks = $this->_module=='Trip' && $this->_controller=='Citybreaksasync';
if($in_city_breaks){
...
<li class="nav-item <?php echo $in_city_breaks ? 'active' : ''; ?> dropdown clickable">
  <a class="nav-link dropdown-toggle" href="<?php echo site_url('trip/citybreaksasync'); ?>" id="cityBreakMN" aria-haspopup="true" aria-expanded="false">City Break</a>
  <div class="dropdown-menu" aria-labelledby="cityBreakMN" style="margin-top: -1px;">
    ...
    foreach ($arival_locations as $location_index =>$arrival_location){
      ...
      ?>
        <a class="dropdown-item <?php echo $in_city_breaks && ($get_location_index == $lindex) ? 'active' : ''; ?>" href="<?php echo site_url('trip/citybreaksasync?arrival=' . $lindex); ?>"><?php echo $option_text; ?></a>
      <?php
      }
    }
    ?>
