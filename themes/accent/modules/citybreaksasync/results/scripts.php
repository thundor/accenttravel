...
function interpretResults(){
  ...
  if(loading_partial_results && !response.total_items){
    setSearchProgress(1);
    $('.rezCount > .filterTitle').text('In curs de cautare oferte in ' + citybreak_search_data.city_name + '.');
  } else{
    if(loading_partial_results){
      $('.rezCount > .filterTitle').text('Am gasit ' + response.total_items + ' oferte in ' + citybreak_search_data.city_name + '. Se continua cautarea...');
    } else {
      $('.rezCount > .filterTitle').text('Am gasit ' + response.total_items + ' oferte in ' + citybreak_search_data.city_name);
    }
  }
...
function loadResults(initial){
  if(!initial && loading_partial_results){
    required_page = citybreak_search_data.page;
    return;
  }
  if(required_page){
    citybreak_search_data.page = required_page;
    required_page = null;
  }

  $('#hotelWarnings').hide();
  $.ajax({
    url: '<?php echo site_url('trip/citybreaksasync/loadResults'); ?>',
    method: 'post',
    dataType: 'json',
    ...
    success: function(result,status,xhr){
      ...
      citybreak_search_data.page = result.results.page;
...
function loadMarkers(){
  $.ajax({
    url: '<?php echo site_url('trip/citybreaksasync/loadMarkers'); ?>',
    method: 'post',
    dataType: 'json',
    data: citybreak_search_data,
    async: true,
...
function loadFilters(){
  $.ajax({
    url: '<?php echo site_url('trip/citybreaksasync/loadFilters'); ?>',
    method: 'post',
    dataType: 'json',
    data: citybreak_search_data,
    async: true,
