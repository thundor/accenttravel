  ...
  $("#addAvionHotel").on("change", function () {
    ...
    if(hotel_search_data.occupancy.length){
      my_obj.o = hotel_search_data.occupancy;
		...
		var filters_link = 'trip/hotels?' + recursiveDecoded;
	...
	function updateCalendar(){
		var caleran = $("#dateHotel").data("caleran");
		caleran.fetchInputs();
	...
	function setSearchAndRedirect(){
		...
		$.ajax({
			url: '<?php echo site_url('trip/hotels/setSearch'); ?>',
			method: 'post',
			dataType: 'json',
			...
			success: function(result,status,xhr){
				...
				window.location.href="<?php echo site_url('trip/hotels/search'); ?>";
			},
