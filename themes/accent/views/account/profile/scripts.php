...
<?php if($can_write){ ?>
  function submitCallback($form,resp,$error_container){
    ...
    if(form.name == 'profilePasswordForm'){
      ...
      form.reset();
      $('.passwordtoggler').trigger('click');
    } else if(form.name == 'profileEmailForm'){
      form.reset();
      $('#current_email_div').text(resp.data.email);
      <?php if($user->type === 'customer'){ ?>
      $('#current_email_input').attr('value',resp.data.email).val(resp.data.email);
      <?php } ?>
      $('.emailtoggler').trigger('click');
      message = 'Email-ul a fost actualizat.';
    }<?php if($user->type !== 'customer'){ ?> else if(form.name == 'profileUsernameForm'){
      form.reset();
      $('#current_username_div').text(resp.data.username);
      $('#current_email_input').attr('value',resp.data.username).val(resp.data.username);
      $('.usernametoggler').trigger('click');
      message = 'Username-ul a fost actualizat.';
    }<?php } ?> else if(form.name == 'profileForm'){
      message = 'Informatiile personale au fost actualizate.';
    } else if(form.name == 'profilePassportForm'){
      message = 'Informatiile despre pasaport au fost actualizate.';
    } else if(form.name == 'invoiceForm'){
      message = 'Informatiile despre facturare au fost actualizate.';
    } else if(form.name == 'profileEmailPreferences'){
      message = 'Preferintele au fost actualizate.';
    }
    showMessage($error_container,message,'success');
    return false;
  }
  $('form.profile_form').on('submit',function(e){e.preventDefault();$('#csrfToken').attr('form',this.name);basicFormPostSubmit(this,"<?php echo site_url('account/save'); ?>",submitCallback);});
  $('input[type=text].input-birth_date').caleran({
    startOnMonday: true,
    startEmpty: true,
    singleDate: true,
    maxDate: moment(),
