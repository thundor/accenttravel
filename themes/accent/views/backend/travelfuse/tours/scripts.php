...
$session_data = $this->_ci->session->userdata('backend/travelfuse/tours');
if (!$session_data) {
  $session_data = array();
}
$session_data = array_replace($default_session_data, $session_data);
?>
<script type="text/twig" id="tours_table_model">
<tr>
	<td class="text-center">{{ tour.Id }}</td>
	<td class="text-center">{{ tour.country }}</td>
	<td class="text-center">{{ tour.namefinal }}{{ tour.namefinal != tour.Name ? '<br /><small>' ~ tour.Name ~ '</small>' : '' }}</td>
	<td class="text-center">{% if tour.status == 1 %}Activ{% elseif tour.status == -1 %}Anulat{% elseif tour.status == -2 %}Sters{% else %}Inactiv{% endif %}</td>
	<td class="unexportable contains-form-control">
		<div class="btn-group">
		{% if tour.can_view %}
			<a class="btn btn-secondary" href="{{ tour.view_link }}"><i class="fa fa-eye"></i></a>
		{% endif %}
		{% if tour.can_edit %}
			<a class="btn btn-primary" href="{{ tour.edit_link }}"><i class="fa fa-pencil"></i></a>
		{% endif %}
		<?php /*
		{% if tour.can_delete %}
			<a class="btn btn-danger delete-tour" title="Sterge" data-status="-2" href="{{ tour.delete_link }}"><i class="fa fa-trash"></i></a>
		{% endif %}
		*/ ?>
		{% if tour.can_change_status %}
			{% if tour.status != 0 %}
			<a class="btn btn-warning unpublish-tour" title="Dezactiveaza" data-status="0" href="{{ tour.unpublish_link }}"><i class="fa fa-thumbs-down"></i></a>
			{% endif %}
			{% if tour.status != 1 %}
			<a class="btn btn-success publish-tour" title="Activeaza" data-status="1" href="{{ tour.publish_link }}"><i class="fa fa-thumbs-up"></i></a>
			{% endif %}
			<?php /*
			{% if tour.status != -1 %}
			<a class="btn btn-inverse archive-tour" title="Anuleaza" data-status="-1" href="{{ tour.archive_link }}"><i class="fa fa-times"></i></a>
			{% endif %}
			*/ ?>
		{% endif %}
		</div>
...
(function($){
  ...
  var $table_tbody = $('table.travelfuse_tours_table>tbody');
  var list_page = 1;
  function getList(){
    var $form_data = {};
    ...
    <?php } ?>
    $.ajax({
		url: "<?php echo site_url('backend/travelfuse/travelfuse_tours/getlist'); ?>",
		method: 'POST',
		dataType: 'json',
		data: $form_data,
		beforeSend: function() {
			$('#travelfuse_tours_wrapper').addClass('loading-state');
		},
		complete: function() {
			$('#travelfuse_tours_wrapper').removeClass('loading-state');
		},
		success: function(msg) {
		  if(msg.status == 'success'){
			...
			var html = '';
			$.each(data.tours, function(){
				var tour = this;
				html += (get_snippet('tours_table_model', {tour:tour}));
			});
			console.warn($('#travelfuse_tours_tbody'), html);
			$('#travelfuse_tours_tbody').html(html);
		  } else {
			alert(msg.message);
		  }
		},
  ...
  $table_tbody.on('click', 'a.delete-tour, a.archive-tour, a.unpublish-tour, a.publish-tour',function(evt){
	 var tour_id = $('>td:nth-child(1)', $(this).closest('tr')).text().trim();
	 var tour_country = $('>td:nth-child(2)', $(this).closest('tr')).text().trim();
	 var tour_name = $('>td:nth-child(3)', $(this).closest('tr')).text().trim();
	 var status = $(this).attr('data-status');
	 var title = 'Confirmare actiune';
	 var text = 'Confirma actiunea: ' + tour_name + ' (' + tour_country + ')';
