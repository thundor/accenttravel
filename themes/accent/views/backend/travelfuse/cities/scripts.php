...
$session_data = $this->_ci->session->userdata('backend/travelfuse/cities');
if (!$session_data) {
  $session_data = array();
}
$session_data = array_replace($default_session_data, $session_data);
?>
<script type="text/twig" id="cities_table_model">
<tr>
	<td class="text-center">{{ city.Id }}</td>
	<td class="text-center">{{ city.country }}</td>
	<td class="text-center">{{ city.namefinal }}{{ city.namefinal != city.Name ? '<br /><small>' ~ city.Name ~ '</small>' : '' }}</td>
	<td class="text-center">{{ city.type }}</td>
	<td class="text-center">{% if city.status == 1 %}Activ{% elseif city.status == -1 %}Anulat{% elseif city.status == -2 %}Sters{% else %}Inactiv{% endif %}</td>
	<td class="unexportable contains-form-control">
		<div class="btn-group">
		{% if city.can_view %}
			<a class="btn btn-secondary" href="{{ city.view_link }}"><i class="fa fa-eye"></i></a>
		{% endif %}
		{% if city.can_edit %}
			<a class="btn btn-primary" href="{{ city.edit_link }}"><i class="fa fa-pencil"></i></a>
		{% endif %}
		<?php /*
		{% if city.can_delete %}
			<a class="btn btn-danger delete-city" title="Sterge" data-status="-2" href="{{ city.delete_link }}"><i class="fa fa-trash"></i></a>
		{% endif %}
		*/ ?>
		{% if city.can_change_status %}
			{% if city.status != 0 %}
			<a class="btn btn-warning unpublish-city" title="Dezactiveaza" data-status="0" href="{{ city.unpublish_link }}"><i class="fa fa-thumbs-down"></i></a>
			{% endif %}
			{% if city.status != 1 %}
			<a class="btn btn-success publish-city" title="Activeaza" data-status="1" href="{{ city.publish_link }}"><i class="fa fa-thumbs-up"></i></a>
			{% endif %}
			<?php /*
			{% if city.status != -1 %}
			<a class="btn btn-inverse archive-city" title="Anuleaza" data-status="-1" href="{{ city.archive_link }}"><i class="fa fa-times"></i></a>
			{% endif %}
			*/ ?>
		{% endif %}
		</div>
...
(function($){
  ...
  var $table_tbody = $('table.travelfuse_cities_table>tbody');
  var list_page = 1;
  function getList(){
    var $form_data = {};
    ...
    <?php } ?>
    $.ajax({
		url: "<?php echo site_url('backend/travelfuse/travelfuse_cities/getlist'); ?>",
		method: 'POST',
		dataType: 'json',
		data: $form_data,
		beforeSend: function() {
			$('#travelfuse_cities_wrapper').addClass('loading-state');
		},
		complete: function() {
			$('#travelfuse_cities_wrapper').removeClass('loading-state');
		},
		success: function(msg) {
		  if(msg.status == 'success'){
			...
			$.each(data.cities, function(){
				var city = this;
				html += (get_snippet('cities_table_model', {city:city}));
			});
			console.warn($('#travelfuse_cities_tbody'), html);
			$('#travelfuse_cities_tbody').html(html);
		  } else {
			alert(msg.message);
		  }
		},
  ...
  $table_tbody.on('click', 'a.delete-city, a.archive-city, a.unpublish-city, a.publish-city',function(evt){
	 var city_id = $('>td:nth-child(1)', $(this).closest('tr')).text().trim();
	 var city_country = $('>td:nth-child(2)', $(this).closest('tr')).text().trim();
	 var city_name = $('>td:nth-child(3)', $(this).closest('tr')).text().trim();
	 var status = $(this).attr('data-status');
	 var title = 'Confirmare actiune';
	 var text = 'Confirma actiunea: ' + city_name + ' (' + city_country + ')';
