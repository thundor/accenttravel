...
$session_data = $this->_ci->session->userdata('backend/travelfuse/facilities');
if (!$session_data) {
  $session_data = array();
}
$session_data = array_replace($default_session_data, $session_data);
?>
<script type="text/twig" id="facilities_table_model">
<tr>
	<td class="text-center">{{ facility.Id }}</td>
	<td class="text-center">{{ facility.namefinal }}{{ facility.namefinal != facility.Name ? '<br /><small>' ~ facility.Name ~ '</small>' : '' }}</td>
	<td class="text-center">{{ facility.type }}</td>
	<td class="text-center">{% if facility.status == 1 %}Activ{% elseif facility.status == -1 %}Anulat{% elseif facility.status == -2 %}Sters{% else %}Inactiv{% endif %}</td>
	<td class="unexportable contains-form-control">
		<div class="btn-group">
		{% if facility.can_view %}
			<a class="btn btn-secondary" href="{{ facility.view_link }}"><i class="fa fa-eye"></i></a>
		{% endif %}
		{% if facility.can_edit %}
			<a class="btn btn-primary" href="{{ facility.edit_link }}"><i class="fa fa-pencil"></i></a>
		{% endif %}
		<?php /*
		{% if facility.can_delete %}
			<a class="btn btn-danger delete-facility" title="Sterge" data-status="-2" href="{{ facility.delete_link }}"><i class="fa fa-trash"></i></a>
		{% endif %}
		*/ ?>
		{% if facility.can_change_status %}
			{% if facility.status != 0 %}
			<a class="btn btn-warning unpublish-facility" title="Dezactiveaza" data-status="0" href="{{ facility.unpublish_link }}"><i class="fa fa-thumbs-down"></i></a>
			{% endif %}
			{% if facility.status != 1 %}
			<a class="btn btn-success publish-facility" title="Activeaza" data-status="1" href="{{ facility.publish_link }}"><i class="fa fa-thumbs-up"></i></a>
			{% endif %}
			<?php /*
			{% if facility.status != -1 %}
			<a class="btn btn-inverse archive-facility" title="Anuleaza" data-status="-1" href="{{ facility.archive_link }}"><i class="fa fa-times"></i></a>
			{% endif %}
			*/ ?>
		{% endif %}
		</div>
...
(function($){
  ...
  var $table_tbody = $('table.travelfuse_facilities_table>tbody');
  var list_page = 1;
  function getList(){
    var $form_data = {};
    ...
    <?php } ?>
    $.ajax({
		url: "<?php echo site_url('backend/travelfuse/travelfuse_facilities/getlist'); ?>",
		method: 'POST',
		dataType: 'json',
		data: $form_data,
		beforeSend: function() {
			$('#travelfuse_facilities_wrapper').addClass('loading-state');
		},
		complete: function() {
			$('#travelfuse_facilities_wrapper').removeClass('loading-state');
		},
		success: function(msg) {
		  if(msg.status == 'success'){
			...
			$.each(data.facilities, function(){
				var facility = this;
				html += (get_snippet('facilities_table_model', {facility:facility}));
			});
			console.warn($('#travelfuse_facilities_tbody'), html);
			$('#travelfuse_facilities_tbody').html(html);
		  } else {
			alert(msg.message);
		  }
		},
  ...
  $table_tbody.on('click', 'a.delete-facility, a.archive-facility, a.unpublish-facility, a.publish-facility',function(evt){
	 var facility_id = $('>td:nth-child(1)', $(this).closest('tr')).text().trim();
	 var facility_name = $('>td:nth-child(3)', $(this).closest('tr')).text().trim();
	 var status = $(this).attr('data-status');
	 var title = 'Confirmare actiune';
	 var text = 'Confirma actiunea: ' + facility_name;
