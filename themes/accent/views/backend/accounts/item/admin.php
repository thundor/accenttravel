...
<?php themeFunctions::loadLang('accounts_item_admin'); ?>
...
foreach($roles as $role){
  if(($role == $user->role) || $this->_ci->user->can('backend-accounts-admin-add-' . $role)){
    $roles_selection[] = $role;
  }
}
?>
<section class="forms">
  <div class="col-12">
    <form id="adminForm" name="adminForm" action="<?php echo site_url('backend/accounts/admin/save'); ?>" method="POST" onsubmit="return false;">
      ...
      <div class="row">
        ...
        <div class="col-lg-4">
          <div class="card">
            ...
            <div class="card-block">
              <div class="form-group row">
                <label for="admin" class="<?php echo $label_class; ?>"><?php echo lang('role_field_label/html'); ?></label>
                <div class="<?php echo $value_class; ?>">
                  <?php if($can_write){ ?>
                  <select id="role" name="role" class="form-control" required>
                    <option value=""><?php echo lang('role_option_none'); ?></option>
                    <?php foreach($roles_selection as $role){ ?>
                      <option value="<?php echo $role; ?>" <?php echo $user->role == $role ? 'selected' : ''; ?>><?php echo lang('role/' . $role); ?></option>
                    <?php } ?>
                  </select>
                  <small class="text-muted"><?php echo lang('role_field_help/html'); ?></small>
                  <?php } else { ?>
                  <div class="form-control" readonly><?php echo $user->role ? lang('role/' . $user->role) : '-'; ?></div>
                  <?php } ?>
                </div>
              </div>
              <div class="form-group row">
      ...
      <div id="result_adminForm" class="form-group"></div>
      <?php 
