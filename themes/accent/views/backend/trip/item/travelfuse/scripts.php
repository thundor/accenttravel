...
(function($){
  ...
  function openOrderService(service_index, $form){
	...
	interpretGetService(service_index,service, $order_service_details);
    $('.lazy',$order_service_details).lazy();
  }
  function interpretGetService(service_index,service_data, $container){
    ...
    var services = service_data?.offer?.Items || [];
    if(services.length){
      for(var i=0; i<services.length;i++){
        var own_service = services[i];
        var $li = $('<li class="list-group-item"/>');
        var price_text = '';
        if(own_service.UnitPrice){
          var price_text = ' (' + format_price(own_service.UnitPrice, own_service?.Currency?.Code || order_currency) + ')';
        }
        $li.addClass(own_service.type).text((own_service?.Merch?.Title || '') + ' ' + '(' + (own_service?.Merch?.type || '') + ((own_service?.Merch?.Code ? " COD: " + (own_service?.Merch?.Code || '') : "")) + ' x ' + (own_service?.Quantity || '') + ')' + price_text + (own_service?.Availability ? ' Availability:' + own_service?.Availability : '') + (own_service?.DepartureDate ? ' DepartureDate:' + own_service?.DepartureDate : '') + (own_service?.ArrivalDate ? ' ArrivalDate:' + own_service?.ArrivalDate : '') + (own_service?.Merch?.TransportType ? ' TransportType:' + own_service?.Merch?.TransportType : '') + (own_service?.Merch?.FromAirport?.Code ? ' FromAirport:' + own_service?.Merch?.FromAirport?.Code : '') + (own_service?.Merch?.ToAirport?.Code ? ' ToAirport:' + own_service?.Merch?.ToAirport?.Code : ''));
    ...
    if(service_data?.offer?.CancelFees?.length){
      for(var i=0;i<service_data?.offer?.CancelFees.length;i++){
        var cancellation_policy = service_data?.offer?.CancelFees[i];
        var penalty_start_date = moment(cancellation_policy.DateStart,'Y-MM-DD HH:mm:ss');
        var penalty_end_date = moment(cancellation_policy.DateEnd,'Y-MM-DD HH:mm:ss');
        if(cancellation_policy.percentage){
          var price_formatted = format_price(cancellation_policy.Price, '%');
        } else {
          var price_formatted = format_price(cancellation_policy.Price, cancellation_policy?.Currency?.Code || order_currency);
        }
        var cancellation_text = "Anularea dupa data " + penalty_start_date.locale('ro').format("DD/MM/Y (dddd, D MMMM)") + " presupune o penalizare de " + price_formatted + ' pana la ' + penalty_end_date.locale('ro').format("DD/MM/Y (dddd, D MMMM)") ;
        var $li = $('<li class="list-group-item"/>');
        $li.text(cancellation_text);
        $li.appendTo($('>.tab-content>.order_service_hotel_cancel_tab .cancellation-policies-list', $card_block));
      }
    }
    if(service_data?.offer?.Installments?.length){
      for(var i=0;i<service_data?.offer?.Installments.length;i++){
        var cancellation_policy = service_data?.offer?.Installments[i];
        var penalty_start_date = moment(cancellation_policy.PayUntil,'Y-MM-DD HH:mm:ss');
        var penalty_end_date = moment(cancellation_policy.PayAfter,'Y-MM-DD HH:mm:ss');
        if(cancellation_policy.percentage){
          var price_formatted = format_price(cancellation_policy.Amount, '%');
        } else {
          var price_formatted = format_price(cancellation_policy.Amount, cancellation_policy?.Currency?.Code || order_currency);
        }
        var cancellation_text = "RATA: Pana la data " + penalty_start_date.locale('ro').format("DD/MM/Y (dddd, D MMMM)") + " se va plati " + price_formatted + ' incepand de la ' + penalty_end_date.locale('ro').format("DD/MM/Y (dddd, D MMMM)") ;
        var $li = $('<li class="list-group-item"/>');
        $li.text(cancellation_text);
        $li.appendTo($('>.tab-content>.order_service_hotel_cancel_tab .cancellation-policies-list', $card_block));
      }
    ...
    $('.service-adults-number', $order_service_hotel_other_tab).text(service_data?.SearchData?.Adults?.[0]);
    $('.service-transport', $order_service_hotel_other_tab).text(service_data?.SearchData?.Transport);
    $('.service-children-number', $order_service_hotel_other_tab).text(service_data?.SearchData?.Children?.[0]);
    $('.service-hotel-checkin', $order_service_hotel_other_tab).text(moment(service_data?.SearchData?.CheckIn,'Y-MM-DD').format('DD.MM.Y'));
	
	if(service_data?.SearchData?.CheckOut)
    $('.service-hotel-checkout', $order_service_hotel_other_tab).text(moment(service_data?.SearchData?.CheckOut,'Y-MM-DD').format('DD.MM.Y'));
	
	if(service_data?.result?.Period)
    $('.service-hotel-period', $order_service_hotel_other_tab).text(service_data?.result?.Period);
    ...
    for(var i=0;i<(service_data?.service_rooms || []).length;i++){
      var room_number = i+1;
      var room = service_data.service_rooms[i];
      ...
      for(var guest_type in room_guests){
        ...
        for(var j=0; j<room_guests[guest_type].length;j++){
          ...
          if(guest_type == 'Adults'){
            ...
            var age = moment().diff(guest.Birthdate, 'years');;
          ...
          $('.guest-title', $guest_model).text(guest.Gender == 2 ? 'F' : 'M');
          $('.guest-lastname', $guest_model).text(guest.Name);
    ...
    var hotel = service_data.result;
    if(hotel?.MainImage?.ExternalUrl)
    $('.hotel-image', $order_service_hotel_info_tab).attr({
      'data-src': hotel.MainImage.ExternalUrl,
    });
    $('.hotel-name', $order_service_hotel_info_tab).text(hotel.Name);
    $('.hotel-location', $order_service_hotel_info_tab).text([hotel?.Address?.City?.Name, hotel?.Address?.City?.County?.Name, hotel?.Address?.City?.Country?.Name].filter(v=>!!v).join(', '));
    $('.hotel-link', $order_service_hotel_info_tab).attr('href','about:blank');
    ...
    $('.hotel-info-description', $order_service_hotel_info_tab).append(hotel?.Content?.Content);
    $('.hotel-info-description', $order_service_hotel_info_tab).append('<h1>Alte informatii</h1>');
    $('.hotel-info-description', $order_service_hotel_info_tab).append('<div style="white-space:pre-line;">' + (hotel.Stages || []).map(v => v?.Content?.ShortDescription).filter(v => !!v).join('<br /><br />') + '</div>');
    if(hotel.Facilities && hotel.Facilities){
      $('.hotel-info-facilities', $order_service_hotel_info_tab).text(hotel.Facilities.join(', '));
    }
