...
<Modal v-model="dialog">
  <template v-slot:activator="{ props }">
    <v-btn v-bind="props" class="rounded-theme w-100 w-100 justify-space-between d-flex py-4 modal-button text-none" append-icon="mdi-chevron-right" variant="outlined">{{ button_text }}</v-btn>
      ...
      <v-list subheader theme="light" class="ma-4 mt-0 max-height mb-4 pe-0" rounded="theme">
        <v-list-item-title class="pa-4 pb-2 text-h5">Selecteaza un pasager</v-list-item-title>
        <v-messages class="pl-4" color="error" :active="!!errors.length" :messages="errors"></v-messages>
        
        <v-list-item theme="dark" v-for="passenger in validPassengers" @click="dialog = false;$emit(passenger.hash && (!(secured || identification_required) || (passenger.secured_correct)) ? 'assign_passenger' : 'open_create_passenger', passenger);">
        ...
        <div class="d-flex align-center px-4">
          <v-btn theme="dark" rounded="theme" color="secondary" class="square-but-inp me-4 flex-grow-0" @click="dialog = false;$emit('open_create_passenger', {})">
            <v-icon>
              mdi-plus
            </v-icon>
          </v-btn>
      ...
      <template v-slot:footer="{ props }">
        <v-btn class="text-none font-weight-normal flex-grow-1 cancel-button v-btn--size-x-large" color="secondary" rounded="theme" @click="dialog = false"><v-icon icon="mdi-arrow-left"></v-icon></v-btn>
      </template>
</Modal>
	`,
    ...
    getValidPassengers(){
      ...
      return this.passengers.filter(v => !v.birth_date || (v.birth_date >= min_date && v.birth_date <= max_date) ).sort((a, b) => {
        ...
        return r;
		  ...
		  if(v.doctype == '1'){
			  v.secured_correct = ((v.ci || '') !== '') && ((v.nationality == 'RO' && (/^[0-9]{13}$/.test(v.ci) && this.validCNP(v.ci))) || (v.nationality != 'RO' && (/^[0-9]{9,20}$/.test(v.ci)))) && ((v.ci_s || '') !== '') && ((v.ci_e || '') !== '') && (v.ci_s < v.ci_e) && (v.ci_s <= d.toISOString().split('T')[0]) && (v.ci_e <= d_max.toISOString().split('T')[0]) && (v.ci_e >= d.toISOString().split('T')[0]);
		  } else {
			  v.secured_correct = ((v.pass || '') !== '') && (/^[0-9]{9,20}$/.test(v.pass)) && ((v.pass_c || '') !== '') && ((v.pass_s || '') !== '') && ((v.pass_e || '') !== '') && (v.pass_s < v.pass_e) && (v.pass_s <= d.toISOString().split('T')[0]) && (v.pass_e <= d_max.toISOString().split('T')[0]) && (v.pass_e >= d.toISOString().split('T')[0]);
		  }
		  return v;
	  });
  ...
  computed: {
    button_text:{
      get() { 
        return this.assigned_passenger ? this.assigned_passenger.firstname + ' ' + this.assigned_passenger.lastname : 'Selecteaza un pasager';
      },
    },
  },
  watch:{
