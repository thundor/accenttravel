export default {
  ...
  <div class="recent-searches" v-if="modelValue && modelValue && modelValue.length">
    <v-list-subheader class="pl-4 mt-2">Destinatii populare</v-list-subheader>
    <v-list
      ...
      get() {
        ...
        if(this.modelValue && this.modelValue){
          this.modelValue.forEach((item) => {
            var i = item.destination;
            if(i && undefined === r.grouped[i.CountryId + '-' + i.CityId + '-' + i.LocationId]){
              r.grouped[i.CountryId + '-' + i.CityId + '-' + i.LocationId] = 1;
              r.results.push(Object.assign({index: r.results.length}, item.destination, {date: new Date(item.date)}));
            }
          })
        }
        return r.results;
