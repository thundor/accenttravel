...
export default {
  ...
  components : {
		'FlightsSearchRecentOrigins' : loadViewAsync('flights_search_recent_origins'),
	},
	data: () => ({
  ...
  <Modal v-model="dialog">
    <template v-slot:activator="{ props }">
      <v-text-field v-bind="props" class="mb-4 mx-4 bg-none"
						label="Plecare din"
						placeholder="Alegeti punctul de plecare"
            readonly
            :active="dialog"
            :focused="dialog"
    ...
    <v-list lines="two" subheader theme="light" class="ma-4 mt-0 max-height pt-4" rounded="theme">
      <v-list-item-title class="pa-4 pb-2 pt-0 text-h5">De unde pleci</v-list-item-title>
      ...
      <div>
        <FlightsSearchRecentOrigins v-model="searches"  v-on:save="saveOrigin" />
      </div>
    </v-list>
  ...
  methods: {
    ...
    save(){
      // console.log('saved origin', this.selected_location);
  ...
  watch:{
    ...
    'selected_location_index': {
      handler(newValue, oldValue){
        // console.warn('selected_location_index', newValue, this.groupedSortedResults[newValue]);
      },
    },
    'modelValue': {
      handler(newValue, oldValue){
