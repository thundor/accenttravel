  ...
  <template v-for="pax_fare in flight_data.FareDetails.PaxFare">
    <template v-for="(pax_split_item, pax_split_index) in parseInt(pax_fare.Count)">
      <div class="bg-background rounded-theme mb-4" style="overflow:hidden;">
        ...
        <div class="pa-4">
          <div class="mb-4">
            <FlightDetailsPassenger :referenceDate="referenceDate" :referenceDateEnd="referenceDateEnd" :identification_required="identification_required" :passport_required="passport_required" v-on:remove_passenger="(p) => removePassenger(p)" v-on:assign_passenger="(p) => addPassenger(p, pax_fare.PTC, pax_split_index)" :secured="secured" :type="pax_fare.PTC" v-on:open_create_passenger="(p) => openCreatePassengerModel(p, pax_split_index, pax_fare.PTC)" :passengers="passengers" :assigned_passengers="assigned_passengers" :assigned_passenger="assigned_passengers[[pax_fare.PTC,pax_split_index]]"></FlightDetailsPassenger>
...
<FlightDetailsCreatePassenger :referenceDate="referenceDate" :referenceDateEnd="referenceDateEnd" :identification_required="identification_required" :passport_required="passport_required" :secured="secured" ref="create_passenger_modal" v-on:save="(p, ptc, cnt) => this.addPassenger(p, ptc, cnt)"></FlightDetailsCreatePassenger>
<FlightDetailsSeatAssignment ref="assign_seat_modal" v-on:save="(p, ptc, cnt, routeIndex, segmentIndex, selectedSeat, defaultSeat) => this.saveSeatAssignment(p, ptc, cnt, routeIndex, segmentIndex, selectedSeat, defaultSeat)" v-on:seat_map="setSeatMap"></FlightDetailsSeatAssignment>
  ...
  mounted: function() {
    ...
    if(pay24Account){
      ...
      if(profile = getObjectDotPathValue(pay24Account,'profile') || {}){
      ...
      if((associated_persons = getObjectDotPathValue(pay24Account,'associated_persons')) && (associated_persons = associated_persons.flat(1)).length){
