...
class PayU extends MX_Controller {
  public function index() {
    ...
    $should_url = site_url('trip/checkout/payu?order_id=' . $order_id);
    ...
    $should_ctrl = hash_hmac('md5',$signature,$settings['payu_secret_key']);
    if($ctrl === $should_ctrl){
  ...
  public function idn($order_id) {
    ...
    $post = array(
      ...
      'ORDER_AMOUNT' => array_sum((array)$gateway_data->IPN_TOTALGENERAL),
    ...
    $post['ORDER_HASH'] = hash_hmac('md5',$signature,$settings['payu_secret_key']);
    $payu_status = (int)$this->Options_model->get('payment_gateways_status','payu',0);
    ...
    // curl_setopt($ch, CURLOPT_CAINFO, __DIR__ . '/secure.payu.ro.crt');
    ...
    if(in_array((int)$arr[1], array(1,7))){
      if(in_array((int)$order->status, array(1))){
        if($order->provider == 'travelfuse'){
          ...
          $booked = false;
          try{
            $book_response = $this->TravelFuse_model->bookOrder($order);
			$order = $this->TripOrder_model->getOrderById($order_id);
			if(empty($order->trip_order_id)){
		  ...
		  $already_booked = $booked_cnt == count($service->Status);
		  if(!$already_booked){
  ...
  public function ipn() {
    ...
    $hash = hash_hmac('md5',$signature,$settings['payu_secret_key']);
    ...
    if(in_array($gateway_status, array('PAYMENT_AUTHORIZED','PAYMENT_RECEIVED','COMPLETE', 'REVERSED', 'REFUND' ))){
      ...
      $this->load->model('TripOrder_model');
      $this->db->where('payment_gateway', 'payu');
      $order = $this->TripOrder_model->getOrderById($order_id);
    ...
    $output_hash = hash_hmac('md5',$signature,$settings['payu_secret_key']);
  ...
  public function checkout($type,$processor_data) {
    $this->data['type'] = $type;
    $this->data['gateway'] = 'payu';
    $this->data['processor_data'] = $processor_data;
    $this->data['payu_payment_method'] = trim($this->input->post('payu_payment_method'));
    $this->theme->view('trip/checkout/online', $this->data, $this);
    return;
  }
  public function validate($type){
    $allowed_payment_methods = $this->Options_model->getKeys('payu_payment_method');
    if(!$allowed_payment_methods || !is_array($allowed_payment_methods)){
      $allowed_payment_methods = array();
    }
    $this->form_validation->set_rules('payu_payment_method', 'Optiune plata PayU', 'trim|required' . ($allowed_payment_methods ? '|in_list[' . implode(',', $allowed_payment_methods) . ']' : ''),array(
