...
class CityBreaksAsync extends MX_Controller {
  ...
  public function index() {
    $this->setData();
    $this->theme->view('trip/citybreaksasync/index', $this->data, $this);
  }
  public function search() {
    ...
    $this->theme->view('trip/citybreaksasync/search', $this->data, $this);
  }
  ...
  protected function inspectSearchIndexHotels() {
    if ($this->input->is_ajax_request()) {
      ignore_user_abort(false);
      static $maxretries = 100;
      if ($maxretries <= 0) {
        $this->outputError('TRIP error: too many retries inspecting search index.');
      }
      if ($maxretries < 100) {
        sleep(1);
      }
      ...
      if (!$this->response) {
        return $this->inspectSearchIndexHotels();
      }
    }
  }
  ...
  protected function initiateFlights() {
    ...
    $this->inspectSearchIndexFlights();
    if (!$this->response) {
  ...
  protected function inspectSearchIndexFlights() {
    if ($this->input->is_ajax_request()) {
      ignore_user_abort(false);
      static $maxretries = 100;
      if ($maxretries <= 0) {
      ...
      $maxretries --;
      $index_id = $this->_getFlightIndex();
      $this->response = $this->Citybreaks_model->inspectSearchIndex($index_id);
      if (!$this->response) {
        return $this->inspectSearchIndexFlights();
      }
    }
  }
  ...
  public function loadResults() {
    ...
    if ($this->input->is_ajax_request()) {
      ...
      $this->response = $this->Citybreaks_model->loadHotels($code, true, $page, $sort_by, $sort_order, $request_filters);
      if (!$this->response) {
        $this->outputError('TRIP error: search results with summary=true returned no response');
      }
      
      $this->results['partial'] = 1;
      if(empty($this->response->summary) || $this->response->summary->progress == 100){
        $this->results['partial'] = 0;
  ...
  protected function interpretHotels() {
    ...
    foreach($this->results['hotels'] as $k => &$hotel){
      ...
      if(!isset($hotel->ShortDesc)){
        $hotel->ShortDesc = '';
      }
      if(empty($hotel->Image)){
        $hotel->Image = site_url('hotelimage.php?id=' . $hotel->Id);
      }
