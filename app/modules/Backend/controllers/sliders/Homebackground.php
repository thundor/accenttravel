...
class Homebackground extends MX_Controller {
  ...
  public function index() {
    ...
    $settings = $this->Options_model->get('sliders_homebackground_settings');
    if(!$settings){
      $settings = array();
    }
    
    $this->data = $settings;
    $this->theme->view('backend/sliders/homebackground', $this->data);
  }
  public function save() {
      ...
      if($uploaded_image){
        ...
        if(!$image){
          ...
          if($image_size_kb > 10 * 1024){
            ...
            $file_deposit_path = $this->theme->theme_path . 'assets/images/bgs/' . $safe_image_name;
            ...
            if(file_exists($file_deposit_path)){
              ...
              // $data['image'][$k] = $safe_image_name;
              $_POST[$fake_post_prefix . 'image'] = 'bgs/' . $safe_image_name;
            }
          }
        }
      }
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      $this->data = $data;
      return $this->theme->view('backend/sliders/homebackground', $this->data);
    }
    
    $this->load->model('Options_model');
    
    $this->Options_model->set('sliders_homebackground_settings',$data);
    $this->redirect('backend/sliders/homebackground', 'Informatiile au fost salvate', 'success');
  }
}