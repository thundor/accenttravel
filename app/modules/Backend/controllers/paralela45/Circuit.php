...
class Circuit extends MX_Controller {
  ...
  public function setSearch($return = false) {
    if ($this->input->is_ajax_request()) {
      ...
      $this->data = $this->Paralela45_Circuit_model->getSearchData($hotel_id);
      ...
      // ));
      $this->form_validation->set_rules('country', 'Tara', 'trim|required|max_length[255]');
      $this->form_validation->set_rules('hotel_name', 'Nume Hotel', 'trim|max_length[255]');
      $this->form_validation->set_rules('origin', 'Oras plecare', 'trim|max_length[255]');
      $this->form_validation->set_rules('destination', 'Oras destinatie', 'trim|max_length[255]');
      $this->form_validation->set_rules('nights', 'Nopti', 'trim|greater_than[0]');
  ...
  public function bookServices() {
    if ($this->input->is_ajax_request()) {
      ...
      try{
        $this->Paralela45_Circuit_model->bookService($order);
      } catch(Exception $e){
  ...
  public function addService(){
    ...
    $this->form_validation->set_rules('package_id', 'ID cautare oferta', 'trim|required|max_length[255]');
    if($id){
