...
class Recommended extends MX_Controller {
  ...
  public function index() {
    ...
    $settings = $this->Options_model->get('offers_recommended_settings');
    if(!$settings){
      $settings = array();
    }
    
    $this->data = $settings;
    $this->theme->view('backend/offers/recommended', $this->data);
  }
  public function save() {
    ...
    foreach($statuses as $i => $status){
      ...
      $data['discount'][$k] = $discount = isset($post['discount'][$i]) ? $post['discount'][$i] : null;
      $data['no'][$k] = $no = isset($post['no'][$i]) ? $post['no'][$i] : null;
      $data['icons'][$k] = $icons = isset($post['icons'][$i]) ? trim($post['icons'][$i], ', ') : null;
      ...
      $_POST[$fake_post_prefix . 'discount'] = $discount;
      $_POST[$fake_post_prefix . 'no'] = $no;
      ...
      $this->form_validation->set_rules($fake_post_prefix . 'no', 'Zona ' . ($k + 1) . ' Numar oferte', 'greater_than_equal_to[0]');
      $this->form_validation->set_rules($fake_post_prefix . 'discount', 'Zona ' . ($k + 1) . ' Discount', 'greater_than_equal_to[0]|less_than_equal_to[100]');
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      $this->data = $data;
      return $this->theme->view('backend/offers/recommended', $this->data);
    }
    
    $this->load->model('Options_model');
    
    $this->Options_model->set('offers_recommended_settings',$data);
    $this->redirect('backend/offers/recommended', 'Informatiile au fost salvate', 'success');
  }
}