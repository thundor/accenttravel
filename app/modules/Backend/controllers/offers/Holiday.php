...
class Holiday extends MX_Controller {
  ...
  public function index() {
    ...
    $settings = $this->Options_model->get('offers_holiday_settings');
    if(!$settings){
      $settings = array();
    }
    
    $this->data = $settings;
    $this->theme->view('backend/offers/holiday', $this->data);
  }
  public function save() {
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      $this->data = $data;
      return $this->theme->view('backend/offers/holiday', $this->data);
    }
    
    $this->load->model('Options_model');
    
    $this->Options_model->set('offers_holiday_settings',$data);
    $this->redirect('backend/offers/holiday', 'Informatiile au fost salvate', 'success');
  }
}