...
class TripNotification_model extends CI_Model {
  function getNotificationById($id, $filters=array()) {
    $filters['id'] = $id;
    return $this->getNotification($filters);
  }
  function getNotificationByCode($code, $filters=array()) {
    $filters['code'] = $code;
    return $this->getNotification($filters);
  }
  function getNotification($filters=array()) {
    $subscribers = $this->getNotifications($filters);
    if($subscribers){
  ...
  function getNotifications($filters = array()) {
    ...
    $q = $this->db->get('trip_notification', $limit, $offset);
  ...
  function getTotalNotifications($filters = array()) {
  ...
  function saveNotification($data) {
    if(isset($data['id']) && $data['id']){
      ...
      $this->db->update('trip_notification', $data);
  ...
  function deleteNotificationById($id) {
    $this->db->where('id', $id);
    $this->db->delete('trip_notification');
  }
}